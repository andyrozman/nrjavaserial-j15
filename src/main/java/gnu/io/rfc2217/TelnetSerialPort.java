...
public class TelnetSerialPort extends SerialPort
{
    ...
    // Modem state bits we always want the server to report to us regardless of
    // what listener wants.
    // This is so we can always stay up-to-date with their values in case
    // isCD(), etc. is invoked.
    ...
    // Line state bits we never want the server to report to us regardless of
    // what listener wants; internally,
    ...
    private int lineStateNotify; // which line state changes we notify listener
                                 // about
    private int lineStateMask; // which line state changes access server
                               // notifies us about
    private int lineStateLast; // most recent line state rec'd from access
                               // server
    private int modemStateNotify; // which modem state changes we notify
                                  // listener about
    private int modemStateMask = MODEMSTATE_ALWAYS_MONITOR; // which modem state
                                                            // changes access
                                                            // server notifies
                                                            // us about
    private int modemStateLast; // most recent modem state rec'd from access
                                // server
    ...
    // We wrap the telnet port's InputStream in a NotifyInputStream so we can
    // detect when there
    ...
    // Method to send serial port "geometry" in the order recommended by RFC
    // 2217 (section 2)
    ...
    // This is invoked by the ComPortOptionHandler when we receive a command
    // from the server
    ...
    void handleCommand(ComPortCommand command)
    {
        ...
        command.visit(new AbstractComPortCommandSwitch()
        {
            ...
            public void caseNotifyModemState(NotifyModemStateCommand command)
            {
                ...
                notify &= modemState ^ modemStateLast; // notify only if bit has
                                                       // changed
    ...
    {
        assert !Thread.holdsLock(TelnetSerialPort.this); // otherwise we can
                                                         // deadlock
        try
        {
    ...
    // Update line state notifications; return true if we need to send new mask
    // to access server
    ...
    // Update modem state notifications; return true if we need to send new mask
    // to access server
